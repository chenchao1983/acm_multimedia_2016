...
The flow chart of the proposed no-reference perceptual quality assessment algorithm is shown in Fig.~\ref{fig:flow_chart}. The noise is first estimated from an input video using the method proposed in \cite{KokKelDenCra2016}. Then, the Power Spectral Densities (PSD) of the noise and the input video are estimated in the frequency domain, respectively (module \circled{1} and \circled{2} in Fig.~\ref{fig:flow_chart}). For a given frequency channel, the ratio between the PSD of the noise and that of the input video (biased by a regularizing constant $\alpha$) are considered as the noise features of that frequency channel (module \circled{3}). These features are further adjusted according to the brightness of the video to incorporate contrast sensitivity (module \circled{4}). A visual importance pooling method is applied to the adjusted features to estimate the visibility of noise in each frequency (module \circled{5}). The final quality metric is then calculated by summing the estimated visibility over the frequencies that show high correlation with subjective quality (module \circled{6}). Details follow. 
\begin{figure*}
...
\subfigure[$\ell_t=2$]{
\includegraphics[width=0.33\textwidth]{./img/corr_3_pub.eps}
}
...
Quality Metrics&SROCC&LCC&VoR\\
\hline
Proposed&\bf{0.9417}&\bf{0.9545}&\bf{16.8952}\\
\hline
